export interface ShopIconDefinition {
  name: string
  label: string
  svg: string
}

export const shopIcons: ShopIconDefinition[] = [
  {
    name: 'gift',
    label: 'Gift',
...
export function getIconSvg(iconName: string): string {
  const icon = shopIcons.find(i => i.name === iconName)
  return icon?.svg || shopIcons[0].svg
}
